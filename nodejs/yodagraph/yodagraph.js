...
const optionDefinitions = [
  ...
  },
  {
    ...
    alias: 'h',
    type: String,
    description: 'Instead of getting a graph, retrieve the content of the indicated HTML tag'
  },
  {
    name: 'help',
    alias: 'h',
...
const sections = [
  {
    header: 'yodagraph',
    content: 'Build a Yoda graph (any graph) using a headless browser and save the image to a file. Alternatively, get any HTML element.'
  },
  {
    header: 'Options',
...
async function run(options) {
  ...
  if (options['html'] == undefined) {
    ...
    beforeContent = await page.evaluate(() => {
      return document.querySelector('#' + options['html']).val();
    });
  }

  let watchDog;
  if (options['html'] == undefined)
    watchDog = page.waitForFunction('document.querySelector("#canvas").height > 400');
  else
    watchDog = page.waitForFunction('document.querySelector('#' + options['html'].val()');
...
try {
  ...
  if (options['help'] == true) {
    ...
    console.log('node yodagraph.js --html RN --file myrn.html --url (release notes URL incl. user/token)');
